    ...
    public void cargarAerolinea( String archivo, Aerolinea aerolinea ) throws IOException, InformacionInconsistenteException
    {
        ...
        for( int i = 0; i < jAeropuertos.length( ); i++ )
        {
            ...
            try
            {
                Aeropuerto aeropuerto = new Aeropuerto( jAeropuerto.getString( "nombre" ), jAeropuerto.getString( "codigo" ),
                        jAeropuerto.getString( "nombreCiudad" ), jAeropuerto.getDouble( "latitud" ), jAeropuerto.getDouble( "longitud" ) );
        ...
        for( int i = 0; i < jRutas.length( ); i++ )
        {
            ...
            if( origen == null || destino == null )
                throw new InformacionInconsistenteException( "La ruta hace referencia a un aeropuerto que no existe" );

            Ruta ruta = new Ruta( origen, destino, jRuta.getString( "horaSalida" ), jRuta.getString( "horaLlegada" ),
        ...
        for( int i = 0; i < jVuelos.length( ); i++ )
        {
            ...
            try
            {
                aerolinea.programarVuelo( jVuelo.getString( "fecha" ), jVuelo.getString( "codigoRuta" ), jVuelo.getString( "avion" ) );
            }
            catch( Exception e )
            {
                throw new InformacionInconsistenteException( e.getMessage( ) );
