...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        {
            if( avionActual.getNombre( ).equals( nombreAvion ) )
            {
                avion = avionActual;
            }
        }
        ...
        int horaInicioNueva = Ruta.getHoras( ruta.getHoraSalida( ) ) * 60 + Ruta.getMinutos( ruta.getHoraSalida( ) );
        int horaFinNueva = horaInicioNueva + ruta.getDuracion( );
        ...
        for( Vuelo vuelo : vuelos )
        {
            if( vuelo.getFecha( ).equals( fecha ) && vuelo.getAvion( ).getNombre( ).equals( nombreAvion ) )
            {
                ...
                int horaInicioExistente = Ruta.getHoras( rutaExistente.getHoraSalida( ) ) * 60 + Ruta.getMinutos( rutaExistente.getHoraSalida( ) );
                int horaFinExistente = horaInicioExistente + rutaExistente.getDuracion( );

                if( horaInicioNueva < horaFinExistente && horaInicioExistente < horaFinNueva )
                {
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        int valorVenta = vuelo.venderTiquetes( cliente, calculadora, cantidad );
        return valorVenta;
    }
